...
export default Component.extend({
	...
	patientCount: computed('salesConfigs', 'overallFilterData', function () {
		...
		let hospitalId = overallFilterData.get('hospitalConfig.hospital.id'),
			...
			treatmentAreaUniqBy = currentSalesConfigs.uniqBy('goodsConfig.productConfig.treatmentArea'),
			patientCount = treatmentAreaUniqBy.reduce((acc, cur) => acc + cur.patientCount, 0);
